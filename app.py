...
if page == "📖 Project Overview":
    st.title("🍷 TrueTaste: AI Wine Grader")
    ...
    # Create an expander so the dataset doesn't take up too much vertical space
    with st.expander("🔍 Click to see Column Definitions & Raw Data"):
        st.write("### What do these columns mean?")
        
        # Complete table explaining all chemical features
        col_desc = {
            "Column Name": [
                "fixed acidity", "volatile acidity", "citric acid", 
                "residual sugar", "chlorides", "free sulfur dioxide", 
                "total sulfur dioxide", "density", "pH", 
                "sulphates", "alcohol", "color", "good"
            ],
            "Description": [
                "Primary acids that contribute to the wine's structure.",
                "The 'Vinegar' factor; high levels lead to an unpleasant taste.",
                "Adds freshness and a hint of citrus flavor to the wine.",
                "Natural sweetness remaining after fermentation stops.",
                "The amount of salt content in the wine.",
                "Prevents microbial growth and oxidation (gas form).",
                "Total amount of SO2; critical for wine preservation.",
                "Mass per unit volume (indicator of sugar and alcohol balance).",
                "Measures acidity level (low pH is acidic, high is basic).",
                "Mineral salts that help preserve wine freshness.",
                "The percentage of alcohol content by volume.",
                "Variety of the wine (Red or White).",
                "Target Variable: 1 for Premium (Quality > 5), 0 for Standard."
            ]
        }
        st.table(pd.DataFrame(col_desc))
        
        # If the red line persists, you can use st.markdown("---") instead
        st.divider() 
...
#     color_val = 1 if color == "White" else 0
    ...
    if st.button("🚀 Analyze Wine Batch"):
       ...
       # --- THE TRIPLE THREAT: UNIQUE FEATURES ---
        ...
        # PREMIUM LOGIC
            with col_feat1:
                st.write("Metric 1") # This works now!
    # ... rest of your code
        else:
    # POOR/STANDARD LOGIC
            with col_feat1:
    ...
    st.write("---") 
    st.header("📊 Model Intelligence Hub")
    st.write("Validation of why Random Forest was chosen as the primary engine.")

    model_tab1, model_tab2 = st.tabs(["Algorithm Comparison", "Quality Metrics"])

    with model_tab1:
        # UPDATED WITH YOUR REAL SCORES
        comparison_data = {
            "Algorithm": ["Random Forest", "KNN", "Decision Tree", "Logistic Regression"],
            "AUC-ROC Score": [0.912, 0.826, 0.809, 0.814],
            "Accuracy": [0.885, 0.833, 0.824, 0.824]
        }
        comp_df = pd.DataFrame(comparison_data)
        
        import plotly.express as px
        fig = px.bar(comp_df, x="Algorithm", y="AUC-ROC Score", color="AUC-ROC Score",
                     text_auto='.3f', title="Winning Algorithm: Random Forest",
                     color_continuous_scale='Viridis')
        st.plotly_chart(fig, use_container_width=True)
        
        st.success("**Technical Verdict:** Random Forest dominates with an **AUC-ROC of 0.912**. This confirms that an ensemble-based approach is best for capturing the complex chemical interactions in wine.")

    with model_tab2:
        col_m1, col_m2 = st.columns(2)
        with col_m1:
            st.write("**Detailed Metrics (Random Forest)**")
            # Using your exact scores
            metrics_data = {
                "Metric": ["Accuracy", "Precision", "Recall", "F1-Score"],
                "Score": ["88.5%", "79.3%", "56.4%", "65.9%"]
            }
            st.table(pd.DataFrame(metrics_data))
        
        with col_m2:
            st.metric("Model Lead", "+8.6%", help="How much better Random Forest is than the baseline (Logistic Regression) in terms of AUC-ROC.")
            st.info("""
            **Precision vs Recall:** Our Precision is high (79.3%), which is intentional. In the luxury wine market, we would rather misclassify a 'Good' wine as 'Standard' (Recall) than sell a 'Standard' wine as 'Premium' (Precision).
            """)
